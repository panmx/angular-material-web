...
export class UserEditComponent implements OnInit {
    ...
    ngOnInit() {
        let user = StorageUtil.getItem(StorageUtil.storageKey.user_detail);
    ...
    /*查询用户信息*/
    findUser() {
        this.userService.findUser(this.saveUser)
            .subscribe(res => {
    ...
    saveUserClick(){
        ...
        let userArr = [];
        userArr.push(this.saveUser);
        this.userService.updateUser(userArr)
            .subscribe(res => {
                this.isLoading = false;
                if (res["success"]) {
                   this.closeCurPage();
