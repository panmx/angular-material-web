...
export class RoleEditComponent implements OnInit {
    ...
    ngOnInit() {
        let role = StorageUtil.getItem(StorageUtil.storageKey.role_detail);
        if(role){
            this.role = role;
        }
        this.calcHeight();
    ...
    dealSelected(){
        let _this = this;
        if(!_this.role.menusList || !_this.role.menusList.length){
            return;
        }
        let filterChild = function (child, menu) {
            child.filter(row=>{
                if(menu.id == row.id){
                    row.addAuthorityBoolean = menu.addAuthority && menu.addAuthority === 1 ? true : false;
                    row.updateAuthorityBoolean = menu.updateAuthority && menu.updateAuthority === 1 ? true : false;
                    row.deleteAuthorityBoolean = menu.deleteAuthority && menu.deleteAuthority === 1 ? true : false;
                    row = _this.transformer(row, 1);
                    let item = row as TodoItemFlatNode;
                    _this.checklistSelection.select(item);
                    // _this.todoLeafItemSelectionToggle(item);
                }
                if(row.childMenus){
                    filterChild(row.childMenus, menu);
                }
            })
        };

        _this.role.menusList.filter(menu=>{
            _this.menusList.filter(row=>{
                if(menu.id == row.id){
                    row.addAuthorityBoolean = menu.addAuthority && menu.addAuthority === 1 ? true : false;
                    row.updateAuthorityBoolean = menu.updateAuthority && menu.updateAuthority === 1 ? true : false;
                    row.deleteAuthorityBoolean = menu.deleteAuthority && menu.deleteAuthority === 1 ? true : false;
        ...
        console.log(_this.checklistSelection.selected)
    }
    ...
    findRoleDetail(){
        this.userService.findRoleDetail(this.role.id)
            .subscribe(res => {
